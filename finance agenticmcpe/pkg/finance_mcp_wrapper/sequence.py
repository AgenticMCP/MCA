...
Server-agnostic — same engine as the github-mcp-server wrapper. A `Step`
declares one tool invocation. Steps run in order. Each step can:
...
* Pull values from a previous step's result via the binding syntax
  ...
  (e.g. ``{"ticker": "$s1.ticker"}`` becomes the string from the prior
  result, not a quoted copy). ``$sN.path`` segments may ALSO be **embedded**
  inside a longer string (e.g. a free-text report line composing several
  earlier outputs): each segment is substituted with the value rendered as
  text (strings as-is, everything else as JSON). Embedded matching is
  restricted to numbered step ids (``$s0``…) with at least one path
  segment, so ordinary text like ``"$scope.foo"`` is never touched; an
  embedded segment that cannot be resolved raises ``BindingError`` instead
  of silently passing the template through.
...
target server actually uses in its tool catalog (type, properties,
...
class Step:
    ...
    ----------
    tool:
        Name of the MCP tool to invoke (e.g. ``"get_historical_stock_prices"``).
    arguments:
        Arguments to pass. Values may be binding templates like
        ``"$s1.ticker"`` that are resolved against earlier step outputs.
    id:
...
def validate_against_schema(
    ...
    *,
    ...
    Supports the subset of JSON Schema actually used by the wrapped MCP
    server's tool catalog: ``type``, ``properties``, ``required``,
