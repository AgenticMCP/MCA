"""CLI for the playwright-agenticmcpe workflow.

    python -m pkg.agenticmcpe config
    python -m pkg.agenticmcpe plan   --task "Open example.com and read the heading"
    ...
    python -m pkg.agenticmcpe selfcheck                  # live observation-only end-to-end
"""
...
def cmd_selfcheck(args: argparse.Namespace) -> int:
    ...
    print(f"[selfcheck] provider={s.llm.provider} model={s.llm.model} "
          ...
          f"server={' '.join(s.server_command)}")
...
def _print_run(result: Any, s: Settings) -> None:
    ...
    if result.replans:
        print(f"           NOTE: plan.json holds only the FINAL plan. The "
              f"consolidated from-scratch sequence is in plan.effective.json; "
              f"earlier attempts are archived as attemptN-plan.json and "
              f"summarized in run.json 'attempts'.")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python -m pkg.agenticmcpe",
                                description="Agentic planner/executor/verifier over playwright-mcp.")
    ...
    # Browser flows lean on the replan loop for runtime-only element refs
    # (observe-then-act), so the default budget is one higher than GitHub's.
    pr.add_argument("--max-replans", type=int, default=3)
    pr.add_argument("--no-verify", action="store_true")
    pr.add_argument("--no-rag", action="store_true",
                    help="Disable knowledge-base retrieval; plan purely from the LLM.")
    pr.set_defaults(func=cmd_run)

    ps = sub.add_parser("selfcheck", help="Live observation-only end-to-end self-check.")
