...
def _pick_archetype(kb: KnowledgeBase, mode: str, category: str | None) -> dict:
    ...
    return min(eligible, key=lambda a: counts.get(a["name"], 0))
...
def _undercovered(kb: KnowledgeBase, catalog: list[dict], mode: str) -> list[str]:
    ...
    pool = [c["tool"] for c in catalog
            ...
            and c["tool"] not in UNSUPPORTED_TOOLS
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Generate GitHub tasks, verify them via agenticmcpe, and "
    ...
    pg.add_argument("--mode", choices=["readonly", "write", "mixed"],
                    ...
                    help="readonly (default, safe) | write (creates "
                         "agenticmcpe-bench-* repos under your account) | mixed.")
