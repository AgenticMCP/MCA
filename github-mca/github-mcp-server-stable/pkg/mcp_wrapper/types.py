"""Data types and exceptions for the github-mcp-server Python wrapper."""
...
class ToolResult:
    ...
    The MCP `content` array contains one or more entries. The github-mcp-server
    tools almost always emit a single text block whose body is JSON. We expose
    the original `content`, the joined raw text, and a best-effort JSON decode.
    """
    ...
    def resource_texts(self) -> list[str]:
        """Text bodies of embedded resource blocks. Some tools (e.g.
        get_file_contents) return the actual file content in a `resource` block,
        while the `text` block is only a status line ("successfully downloaded
        ...")."""
        out: list[str] = []
    ...
    def data(self) -> Any:
        ...
        resource content (the file body for get_file_contents), then the raw
        text, then None.
        """
