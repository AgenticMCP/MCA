...
# Any "$<step_id>" reference — the whole value ("$s1", "$s1.items[0].id") or
# embedded in a longer string ("repo:$s1.full_name is:open", the executor's
# embedded form). One pass over the string, so a renamed id is never renamed
# twice; ids not in the map (prose like "$scope") are left alone.
_STEP_REF_RE = re.compile(r"\$(?P<step>[A-Za-z_][A-Za-z0-9_-]*)")


def _rewrite_bindings(value: Any, idmap: dict[str, str]) -> Any:
    """Rewrite "$sN..." references inside ``value`` per ``idmap``, whether the
    binding is the whole value or embedded in a longer string."""
    if isinstance(value, str):
        return _STEP_REF_RE.sub(
            lambda m: f"${idmap.get(m.group('step'), m.group('step'))}", value)
...
def merge_effective_plan(executed: list[tuple[Plan, ExecutionTrace]]) -> Plan | None:
    ...
    arguments — e.g. the re-included get_me of a replan). Failed/skipped steps
    are excluded; their corrected replacements come from the later attempt.
    """
    if len(executed) < 2:
        return None
...
def _effect_hint(tool: str, args: dict[str, Any], data: Any) -> str:
    ...
    exists (don't recreate). For reads: a preview of the returned value, so the
    planner can correct paths/bindings on replan."""
    if tool == "get_me" and isinstance(data, dict):
        return f"login={data.get('login')}"
    if tool == "create_repository":
        return f"repository '{args.get('name')}' now EXISTS"
    if tool == "create_branch":
        return f"branch '{args.get('branch')}' now EXISTS"
    if tool in ("create_or_update_file", "push_files"):
        path = args.get("path") or args.get("files")
        return f"wrote {_short(path)} on branch '{args.get('branch')}'"
    if tool == "create_pull_request":
        # Include the PR number (the executor surfaces it) so a replan binds to
        # the existing PR instead of opening a duplicate.
        num = data.get("number") if isinstance(data, dict) else None
        return (f"pull request #{num} now EXISTS — reference this number, do NOT open another"
                if num is not None else "pull request now EXISTS — do NOT open another")
    # Idempotency hints for replans: issues and comments duplicate SILENTLY on
    # GitHub, so the planner must reference what already exists, not re-create.
    if tool == "issue_write" and isinstance(data, dict) and data.get("number") is not None:
        verb = "was UPDATED" if args.get("method") == "update" else "now EXISTS"
        return (f"issue #{data['number']} {verb} — bind/reference this number, "
                f"do NOT create it again")
    if tool == "add_issue_comment":
        return f"comment already POSTED on issue #{args.get('issue_number')} — do NOT post it again"
    # Gists also duplicate silently — surface the id so a replan references it.
    if tool == "create_gist":
        gid = data.get("id") if isinstance(data, dict) else None
        return (f"gist {gid} ({_short(args.get('filename'))}) now EXISTS — "
                f"reference this id, do NOT create another" if gid
                else "gist now EXISTS — do NOT create another")
    if tool == "sub_issue_write" and args.get("method") == "add":
        return (f"sub-issue {args.get('sub_issue_id')} already ATTACHED to issue "
                f"#{args.get('issue_number')} — do NOT add it again")
    # read tools: surface a preview so the planner can fix wrong paths/shapes
    if tool == "get_file_contents" and isinstance(data, dict):
        return (f"returned content: {_short(data.get('content'), 160)} "
                f"sha={data.get('sha')}")
    if isinstance(data, str):
        return f"returned STRING: {_short(data, 180)}"
    if isinstance(data, dict):
        hint = f"returned object keys={list(data.keys())[:6]}"
        return hint + (f" sha={data['sha']}" if "sha" in data else "")
...
def _replan_feedback(plan: Plan, trace: ExecutionTrace) -> str:
    ...
    for ps in plan.steps:
        ...
        lines.append(line)
    lines.append("")
    lines.append("SUCCESS steps and their resources already EXIST — do not recreate them. "
                 "Re-plan from the first FAILED step onward for the original task.")
    return "\n".join(lines)
...
# Default self-check task: read-only, safe to run against live GitHub.
SELFCHECK_TASK = (
    "Look up my own GitHub profile, then list up to 5 open issues in the "
