"""CLI for the agenticmcpe workflow.

    python -m pkg.agenticmcpe config
    python -m pkg.agenticmcpe plan   --task "Create an issue titled X in owner/repo"
...
def cmd_toolsource(args: argparse.Namespace) -> int:
    ...
    print(f"[toolsource] {covered}/{len(index['tools'])} tools mapped to Go "
...
def cmd_selfcheck(args: argparse.Namespace) -> int:
    ...
    if not s.tokens.available:
        print("  ERROR: no GitHub token. Set GITHUB_PERSONAL_ACCESS_TOKEN in env/.env.",
              file=sys.stderr)
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python -m pkg.agenticmcpe",
                                description="Agentic planner/executor/verifier over github-mcp-server.")
    ...
    pt = sub.add_parser("toolsource",
                        help="Build the tool->Go source/test JSON KB the "
