...
_BENIGN_LITERALS = {
    "main", "master", "readme.md", "readme", "open", "closed", "all", "true",
    "false", "head", "origin", "asc", "desc", "created", "updated",
}


# Git ref prefix: "refs/heads/main" names the same thing as "main", so ground
# the branch part and ignore the wrapper.
_REF_PREFIX_RE = re.compile(r"^refs/(?:heads|tags)/")
...
def task_grounds_entry(
    ...
    stored sequence: the stored plan's concrete inputs (owners, repos, paths,
    titles, file contents) must all be present in THIS task, so replaying them is
    correct rather than a leftover from the original task. Bindings, very short
    tokens and benign scaffolding literals (main, README.md, open, ...) are
