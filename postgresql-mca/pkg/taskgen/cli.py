"""CLI for the taskgen benchmark builder.
    ...
    python -m pkg.taskgen generate --count 3 --mode readonly        # full loop
...
from .generator import (ARCHETYPES, DIFFICULTY_STEPS, GENERATION_TEMPERATURE,
                        TaskGenError, TaskGenerator, unsupported_tools)
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    # The generator needs a work dir only to cache the tool catalog — use ONE
    # stable dir instead of a fresh timestamped run dir per invocation (which
    # littered runs/ with catalog-only directories that look like empty task
    # runs), and get the catalog-cache hit for free.
    gen_settings = Settings.load(provider=args.provider, run_id="taskgen-catalog")
    ...
    for i in range(args.count):
        mode = _pick_mode(args.mode, i)
        difficulty = (args.difficulty if args.difficulty != "mixed"
                      else _DIFF_CYCLE[i % len(_DIFF_CYCLE)])
        archetype = _pick_archetype(kb, mode, args.category)
        print(f"\n=== task {i + 1}/{args.count} "
              f"[{archetype['name']} | {mode} | {difficulty}] ===")
        try:
            spec = generator.generate(
                archetype,
                difficulty=difficulty,
                undercovered=_undercovered(kb, catalog, mode),
...
def _pick_mode(mode: str, i: int) -> str:
    if mode == "mixed":
        return "readonly" if i % 2 == 0 else "write"
    return mode


def _pick_archetype(kb: KnowledgeBase, mode: str, category: str | None) -> dict:
    if category:
        for a in ARCHETYPES:
            if a["name"] == category:
                return a
    ...
    eligible = [a for a in ARCHETYPES if a["write"] == (mode == "write")]
    counts = kb.category_counts()
    # Coverage-driven, but sampled among the few least-covered rather than a
    # deterministic min: an archetype that systematically FAILS never gains
    # coverage, so a strict min re-picks it every single turn and starves the
    # rest (observed live: 3 archetypes took 49 of 60 consecutive rejects).
    ranked = sorted(eligible, key=lambda a: counts.get(a["name"], 0))
    return random.choice(ranked[:4])


def _undercovered(kb: KnowledgeBase, catalog: list[dict], mode: str) -> list[str]:
    unsupported = unsupported_tools()
    covered = kb.tool_coverage()
    pool = [c["tool"] for c in catalog
            if (mode == "write" or c["read_only"])
            and c["tool"] not in unsupported
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Generate PostgreSQL tasks, verify them via agenticmcpe, "
                    "and grow a task->tool-sequence knowledge base.")
    ...
    pg.add_argument("--mode", choices=["readonly", "write", "mixed"],
                    default="readonly",
                    help="readonly (default, safe) | write (creates "
                         "agenticmcpe_bench_* schemas in the target database) "
                         "| mixed.")
    ...
    pg.add_argument("--cleanup", action="store_true",
                    help="Drop agenticmcpe_bench_* schemas created by write "
                         "tasks (DROP SCHEMA ... CASCADE).")
