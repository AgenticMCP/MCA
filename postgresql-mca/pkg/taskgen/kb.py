"""JSON knowledge base of verified task -> tool-call-sequence mappings.

One file, append-only entries, atomic writes. Each entry pairs a task prompt
and summary with the tool-call sequence that was proven correct by execution +
verification, so the file can be loaded directly as a RAG corpus for the
planner ("here are solved tasks similar to yours").
"""
...
class KnowledgeBase:
    ...
    def has_prompt(self, prompt: str) -> bool:
        ...
        return any(str(e.get("task_prompt", "")).strip().casefold() == norm
                   for e in self.entries)
    ...
    def stats(self) -> dict[str, Any]:
        return {
            ...
            "by_mode": dict(Counter(e.get("mode", "?") for e in self.entries)),
