...
except ImportError:  # vendored standalone — mcpuniverse isn't installed
    import logging

    def get_logger(name: str) -> logging.Logger:
        """Stdlib fallback. Logs to stderr: stdout carries JSON-RPC frames."""
...
def _log(*args, **kwargs):
    ...
    Under ``--transport stdio`` stdout carries the JSON-RPC frame stream —
    a bare ``print`` there splices non-JSON text into the protocol and
    kills the session.
    """
